...
export async function getAthleteDetail(id: string): Promise<AthleteDetail | null> {
  ...
  const overlay = (s: typeof plannedSessions.$inferSelect): DisplaySession => {
    ...
    return {
      ...
      targetDurationSeconds: adj.durationSeconds ?? null,
      targetPowerLowWatts: adj.adjustments.length ? null : s.targetPowerLowWatts,
      targetPowerHighWatts: adj.adjustments.length ? null : s.targetPowerHighWatts,
      segments: adj.adjustments.length ? null : s.segments,
      description: adj.adjustments.length ? adj.adjustments.join("; ") : s.description,
